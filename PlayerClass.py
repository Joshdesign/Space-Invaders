...
class Player(Ship):
    def __init__(self, x_speed, y_speed, health=100):
        super().__init__(x_speed, y_speed, health)
    ...
    def move(self):
        ...
        if (keys[pygame.K_UP] or keys[pygame.K_w]) and (self.y > 0):
            self.y -= self.y_speed
        elif (keys[pygame.K_DOWN] or keys[pygame.K_s]) and (self.y < HEIGHT - self.get_height()):
            self.y += self.y_speed
    ...
    def fire(self, window):
        ...
        for i in range(len(self.fired_bullets)):
            self.fired_bullets[i].move(self.bullet_spedd)
